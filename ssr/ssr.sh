...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://cdn.jsdelivr.net/gh/colaly/config/ssr/manyuser.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://gitee.com/colalp/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://cdn.jsdelivr.net/gh/colaly/config/ssr/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://cdn.jsdelivr.net/gh/colaly/config/ssr/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
Install_ServerSpeeder(){
	...
	cd /root
	#借用91yun.rog的开心版锐速
	wget -N --no-check-certificate https://cdn.jsdelivr.net/gh/colaly/config/ssr/serverspeeder.sh
